...
<script>
	...
	function matchStatus() {
		if ($match.testHighlight.filter(item => item.className !== "no-match").length == 0) {
			...
			return `${
				$match.testHighlight.filter(item => item.className !== "no-match").length
			} ${$editor.getMatchStatus()} - ${$match.result.time} ms`;
		}
	}
