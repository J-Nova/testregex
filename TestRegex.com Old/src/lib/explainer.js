export var Regex101Explainer = function () {
            ...
            if (a.type === regex_type.CHARCLASS) 
                for (var n = 1; n < a.children.length - 1; n++) {
                    ...
                    if (s.type === regex_type.RANGE_HYPHEN) {
                        ...
                        i.type === regex_type.TEXT ? (h = i.contents.slice(-1), i.contents = i.contents.slice(0, -1), "" === i.contents && (a.children.splice(n - 1, 1), n --)) : (a.children.splice(n - 1, 1), n --),
                        -1 !== o && (c.type === regex_type.TEXT ? (p = c.contents.charAt(0), c.contents = c.contents.slice(1), "" === c.contents && a.children.splice(n + o, 1)) : a.children.splice(n + o, 1)),
                        s.contents = h + s.contents + p,
                        s.range_from = h,
                        s.range_to = p
                    }
                }
    ...
    function t(e) {
        for (var r, a = 0; a < e.length; a++) {
            var n = e[a];
            n.type !== regex_type.TEXT || n.quantifier || /^(?:\r\n|\r|\n)$/.test(n.contents) ? r = void 0 : void 0 === r ? r = n : (r.contents += n.contents, e.splice(a, 1), a --),
    ...
    function r(e, t) {
        for (var a =! 1, n = 0; n < e.length; n++) {
            ...
            t.type === regex_type.CONDITIONAL ? (s.branch =! 0, e.shift(), e.unshift(s)) : i.type !== regex_type.ALTERNATOR || a || (e.unshift(s), a =! 0),
            isGroupType(i.type) && r(i.children, i)
        }
    }
    function a(e, t) {
        ...
        for (var r = "", a = 1, i = 0; i < e.length; i++) {
            ...
            if (isGroupType(o.type)) {
                switch (r += D(d(o)), r += "<ul>", f && (r += M(C("Quantifier:") + " " + T(o.quantifier) + " " + f, !1), "{0}" !== o.quantifier && "{1}" !== o.quantifier && "{0,0}" !== o.quantifier && "{1,1}" !== o.quantifier && (o.type === regex_type.NAMED_GROUP || o.type === regex_type.NAMED_P_GROUP || "(" === o.contents) && (r += M(F(P("Note:") + " A repeated capturing group will only capture the last iteration. Put a capturing group around the repeated group to capture all iterations or use a non-capturing group instead if you're not interested in the data"), !1))), o.type) {
                    case regex_type.SUBPATTERN_GROUP: o.currentCapturingGroupCount = V,
                        r += M(F(P("Note:") + " Subpatterns declared within each alternative of this construct will start over from the same index"), !1);
                        break;
                    case regex_type.GROUP_MODIFIER:
                        ...
                        break;
                    case regex_type.CONDITIONAL: r += D("Evaluate the condition below and proceed accordingly"),
                        r += "<ul>",
                        r += M(s(o.condition, o), !1),
                        r += "</ul>"
                }
                r += o.children.length > 0 ? n(o.children, o) : M("(null, matches any position)"),
                r += "</ul>"
            } else if (o.type === regex_type.CHARCLASS) 
                r += D(c(o)),
                r += "<ul>",
                f && (r += M(C("Quantifier:") + " " + T(o.quantifier) + " " + f, !1)),
                W == "JAVASCRIPT" && "[^]" === o.group_contents ? r += F(P("Note:") + " Avoid this construct, use " + b(".") + " or " + N("[\\s\\S]") + " instead.") : o.children && (r += n(o.children, o)),
                r += "</ul>";
                ...
                if (0 === m.length && (m = "(null, matches any position)"), r += D(t.type === regex_type.CONDITIONAL ? L(o.branch ? "TRUE" : "FALSE", o.branch ? 1 : 2) + ": " + k(escapeHtml(m)) : S(ord(a++)) + " Alternative: " + k(white_space(escapeHtml(m)))), r += "<ul>", o.children.length > 0) 
                    r += n(o.children, o);
                 else {
                    var y = " An empty alternative effectively truncates the regex at this point because it will always find a zero-width match";
                    isGroupType(t.type) && (y = " An empty alternative effectively makes this group optional which suggests the alternative is completely redundant"),
                    r += M(F(P("Warning:") + y), !1)
                } r += "</ul>"
            } else if (o.type === regex_type.QUOTE) 
                "\\Q" === o.contents ? (r += D("Begin " + b("\\Q...\\E") + " sequence"), r += "<ul>") : r += "</ul>";
                ...
                if (void 0 !== o.modifiers && -1 !== o.modifiers.indexOf("x") && "" === o.contents.replace(/\s+/g, "")) 
                    continue;
    ...
    function s(e, t) {
        if (regex_type.LOOKBEHIND === e.type || regex_type.LOOKAHEAD === e.type) 
            return n([e], t);
    ...
    function c(e) {
        ...
        return W == "JAVASCRIPT" && "[^]" === e.group_contents ? t += " match any character" : (t += " match a single character ", t += "^" === e.contents.charAt(1) ? "not present" : "present", t += " in the list below"),
        t
    }
    function o(e) {
        var t = e.replace("^", "");
    ...
    function h(e, t) {
        ...
        if (a) {
            ...
            switch ("?" === i && (c = s ? "many" : "few"), n) {
                case "*": r += O("zero") + " and " + O("unlimited") + " times, as " + c + " times as possible";
                    break;
                case "+": r += O("one") + " and " + O("unlimited") + " times, as " + c + " times as possible";
                    break;
                case "?": r += O("zero") + " and " + O("one") + " time, as " + c + " times as possible";
                    break;
                case "{":
                    var o = e.from,
                        h = e.to;
                    if (o === h) {
                        var p = "Exactly " + O(o) + " time" + (
                        o > 1 || 0 == o ? "s" : ""
                    );
                        return 0 == o ? p += " (causing the token to be ignored)" : 1 == o && (p += " (meaningless quantifier)"),
                        p
                    }
                    h || (h = "unlimited"),
                    r += O(o) + " and " + O(h) + " times, as " + c + " times as possible"
            }
    ...
    function u(e, t) {
        ...
        switch (s && e.type !== regex_type.ESCAPED_TEXT && (m = m.replace(/\s/g, "")), e.type) {
            ...
            case regex_type.META:
                if (/^\\u.{4}$/.test(p)) 
                    return b(y) + "matches the Unicode character " + A(white_space(escapeHtml(String.fromCodePoint(parseInt(p.slice(2), 16)))));
                ...
                return /^\\c/.test(p) ? b(y) + " matches " + P("Ctrl+" + escapeHtml(p.substring(2))) + " " + C("(ASCII " + (
                    64 ^ p.substring(2).toUpperCase().charCodeAt(0)
                ) + ")") : "." === p ? b(y) + " matches any character" + (
                a ? "" : " (except newline)"
            ) + (n ? " " + C("[unicode]") : "") : "^" === p ? b(y) + " assert position at start of " + (
                i ? "a line" : "the string"
            ) : "$" === p ? b(y) + " assert position at end of " + (
                i ? "a line" : "the string"
            ) : "\\b" === p ? d ? b(y) + " a backspace character " + P("(ASCII 8)") : b(y) + " assert position at a word boundary " + P("(^\\w|\\w$|\\W\\w|\\w\\W)") : "\\B" === p ? d ? u({
                    contents: p,
            ...
            case regex_type.TEXT:
                ...
                if (d) {
                    var T = escapeHtml(_.uniq(p).join("")),
                        k = b("" === y ? O : y);
                    return k += T.length > 1 ? " a single character in the list " + A(replaceBackslash(T)) + " literally" : " the literal character " + A(replaceBackslash("" === T ? O : T)),
                    k += l(T, c)
                }
                if ("\n" === h) 
                    return b("&not;") + " matches a line-feed (newline) character " + P("(ASCII 10)");
                
                if ("\r" === h) 
                    return b("&not;") + " matches a carriage return " + P("(ASCII 13)");
                
                var S = "" === m ? O : m;
                e.type !== regex_type.QUOTE_TEXT && (S = replaceBackslash(S));
                var G = b("" === y ? O : y) + " matches the character" + (
                m.length > 1 ? "s " : " "
            ) + A(S) + " literally";
                return G += l(h, c);
            ...
            case regex_type.MODIFIERS:
                var H = "";
                return H += D(E(y) + " Match the remainder of the pattern with the following options:"),
                H += "<ul>",
                H += /^\(\?-*\)$/.test(p) ? M(F(P("Note:") + " Empty group with no modifiers specified - very useless"), !1) : g(p.slice(2, -1)),
                H += "</ul>";
    ...
    function d(e) {
        ...
        if ("(" === t) 
            return V++,
            L(ord(V), V) + " Capturing group " + E(r);
        
        if (e.type === regex_type.CONDITIONAL) 
            return "IF Clause " + w(r);
        
        if (e.type === regex_type.DEFINE) 
            return "DEFINE construct " + w(r) + " (only for reference)";
    ...
    function L(e, t) {
        return y(e, "capturinggrooup match" + (
            t % 10 || 10
        ))
    }
    function D(e) {
    ...
    function M(e, t) {
        var r = "file";
        return t && (r += " treeview_bold"),
        '<li><div class="' + r + '">' + e + "</div></li>"
    }
    function F(e) {
    ...
    Z.explain = function (i) {
        W = i.flavor ? i.flavor : "PCRE";
        var s;
        switch (W) {
            case "JAVASCRIPT": s = /^[img]$/,
                j = "Javascript";
                break;
            case "PYTHON": s = /^[imgsxu]$/,
                j = "Python";
                break;
            case "PCRE":
            default: s = /^[xXsiuUmgAD]$/,
                j = "PCRE (PHP)"
        }
        var c = Regex101Colorizer.returnTokens(),
            o = "<li>";
        Regex101Colorizer.hasError() || (o += '<div class="hitarea collapsable"><i class="fa fa-caret-down"></i></div>'),
        o += '<div class="first-folder" class="folder"><div><span class="expslash">' + escapeHtml(i.delimiter) + '</span><span class="treeview_pattern regex_colorizer">' + Regex101Colorizer.makeHtml(c[0].children, !0, !0) + '</span><span class="expslash">' + escapeHtml(i.delimiter) + '</span><span class="treeview_pattern treeview_options">' + escapeHtml(i.options) + "</span></div></div>";
        var h = '<ul id="regexExplainer" class="filetree treeview">' + o + "<ul>";
        if (Regex101Colorizer.hasError()) {
            h += '<li class="treeview_error_root">Errors are explained from left to right. Move the mouse cursor over them to see the error highlighted in your pattern</li>',
            h += m(c[0].children);
            for (var p = _.uniq(i.options), l = i.options, u = 0; u < p.length; u++) 
                s.test(p[u]) || (h += '<li class="treeview_error">' + I(p[u]) + "-modifier does not exist in   (or is not yet supported by regex101.com)</li>"),
                l = l.replace(p[u], "");
            
            l = _.uniq(l);
            for (var u = 0; u < l.length; u++) 
                h += '<li class="treeview_error">Duplicate modifier: ' + I(l[u]) + "</li>"
            
        } else 
            V = 0,
            $ = Regex101Colorizer.getCaptureData(),
            e(c[0].children),
            r(c[0].children, c[0]),
            a(c[0].children, c[0]),
            t(c[0].children),
            h += n(c[0].children, c[0]),
            h += g(i.options);
        
        return h += "</ul></ul>"
    },
    Z.quickTokenExplain = function (e, t) {
        return u({token: e, type: t})
    },
    Z.explain_options = function (e) {
        return K ? "<li>Your pattern contains an error, please fix it first.</li>" : g(e)
    },
    Z.explainError = function (error_code, token) {
        switch (error_code) {
            ...
            case error_type.UNSUPPORTED_TOKEN:
                return void 0 === token ? "This token is not supported in the selected flavor" : "The token " + b(token.contents) + " is not supported in " + j;
            ...
            default:
                return " No error message for this it seems. Please create a permalink and submit to contact@regex101.com"
        }
    },
    Z.quickExplain = function (e, t, r) {
        for (var a = "", n = 0; n < e.childNodes.length; n++) 
            3 === e.childNodes[n].nodeType && (a += e.childNodes[n][textMethod]);
        ...
        switch (t) {
            ...
            case regex_type.GROUP_QUANTIFIER:
                ...
                switch (a.charAt(0)) {
                    case "+": c += "<i>one</i> to <i>infinite</i> times";
                        break;
                    case "*": c += "<i>zero</i> to <i>infinite</i> times";
                        break;
                    case "?": c += "<i>one</i> or <i>zero</i> times";
                        break;
                    case "{":
                        ...
                        if (2 !== o.length) 
                            return c += "exactly <i>" + o[0] + "</i> times";
                        
                        c += "<i>" + o[0] + "</i> to <i>" + (
                        o[1] ? o[1] : "infinite"
                    ) + "</i> times"
                }
                if (a.length > 1 && "}" !== a.slice(-1)) 
                    switch (a.slice(-1)) {
                        case "+": c += ", without giving back";
                            break;
                        case "?":
                            if (! s) {
                    ...
                    c += s ? ", as few times as possible" : ", as many times as possible";
            ...
            case regex_type.REFERNCE_GROUP:
                ...
                if ("(?R)" === a || "(?0)" === a) 
                    return p + "Recurses the entire pattern";
                
                if (-1 !== Regex101Colorizer.getCaptureData().subpatterns.indexOf(o)) 
                    return p + "Subpattern reference to group named `<i>" + o + "</i>`";
                
                var l = o.charAt(0);
                return "+" === l || "-" === l ? p + "Reference to the <i>" + ord(parseInt(o.slice(1))) + "</i> subpattern declared, seen from the " + (
                "+" === l ? "right" : "left"
            ) + " of this token" : p + "Reference to the <i>" + ord(parseInt(o)) + "</i> subpattern";
            case regex_type.OCTAL:
            ...
            case regex_type.LOOKBEHIND:
                switch (a) {
                    ...
                    default:
                        return "<b>Lookbehind.</b> Start of " + (
                        -1 === a.indexOf("!") ? "positive" : "negative"
            ...
            case regex_type.META:
                switch (i) {
                    case "^":
                        return "<b>Anchor.</b> Anchor to beginning of " + (
                        -1 === r.indexOf("m") ? "string" : "line"
                    );
                    case "$":
                        return "<b>Anchor.</b> Anchor to end of " + (
                        -1 === r.indexOf("m") ? "string" : "line"
                    );
                    case ".":
                        return "<b>Dot.</b> Matches any character" + (
                        -1 === r.indexOf("s") ? " except newline" : ""
                    );
                    case "\\G":
