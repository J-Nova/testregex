...
function newMatch(_) {
    if (debugging) {
        var t;
        t = match_steps > 0 ? _ >= 0 ? "Match found in " + (
            match_steps + 1
        ) + " step(s)" : _ === PCRE_ERROR_NOMATCH ? "Match failed in " + (
            match_steps + 1
        ) + " step(s)" : _ === PCRE_ERROR_MATCHLIMIT ? "Catastrophic backtracking was detected and execution halted. See: <a href='http://www.regular-expressions.info/catastrophic.html'>Runaway Regular Expressions</a>" : "Funky error: " + _ : "Optimization caused halt.",
        callout_data[match_id][match_steps] = {
            pstart: oldPatternStart,
            pend: oldPatternEnd,
            sstart: oldStrStart,
        ...
        do {
            ...
            if (o >= 0) {
                ...
                for (var d =[], p = 0, N = 0; 2 * o > N; N += 2) {
                    ...
                    if (i) {
                        if (I === u) 
                            putPrefix(A, u, '<span class="match_99">&nbsp;</span>');
                        else {
                            var h = "",
                                m = n.length + ";" + I + ";" + u + ";" + N / 2;
                            if (0 === N) 
                            ...
                            if (R > 0) 
                                for (var L = HEAP32[a >> 2], f = 0; R > f; f++) {
                                    var M = HEAP8[L];
                                    void 0 !== d[M] && (d[M].name = UTF16ToString(L + 2), M === p - 1 && (F += d[M].name)),
                                    L += 2 * E
                                }
                            
                            m += F,
                            putSuffix(A, I, '<span class="' + h + '" data-tooltip="' + m + '" data-groupNum="' + N / 2 + '">'),
                            putPrefix(A, u, "</span>")
                        }
                        0 === N && (T[O++] =
...
self.onmessage = function (_) {
    debugging = _.data.debugging,
    self.postMessage("onload")
    try {
        preg_compile(_.data.regex, _.data.options)
        self.postMessage(preg_match(_.data.regexText))
    } catch (e) {
        // TODO Handle errors more gracefully and showing the user the error properly.
        console.log("Error compiling expression", e);
    }
};
