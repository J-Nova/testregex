...
function setFlavor(e) {
    var t = "substitution; \\num = backreference, \\n = newline, \\t = tab";
    switch (debugHtml = debugData =
        {}, e) {
        case FLAVOR.JS:
            $(".slash_menu").attr("data-dropdown", ""),
            $("#sub").attr("placeholder", t.replace("\\num", "$num")),
            $("#options").attr("placeholder", "gmi"),
            $(".slash").text("/"),
            window.location.hash = "javascript",
            $("#debugger_menu, #format_regex").hide();
            break;
        case FLAVOR.PCRE:
            $(".slash_menu").attr("data-dropdown", ".delimiter-dropdown"),
            $("#options").attr("placeholder", "gmixXsuUAJ"),
            $("#sub").attr("placeholder", t),
            /^([\/~@;%`])\1$/.test($(".slash").text()) || $(".slash").text("/"),
            window.location.hash = "pcre",
            $("#format_regex, #debugger_menu").show(),
            $("#debugger_menu").addClass("disabled");
            break;
        case FLAVOR.PYTHON:
            $("#sub").attr("placeholder", t),
            $("#options").attr("placeholder", "gmixsu"),
            $(".slash").text('"'),
            $(".slash_menu").attr("data-dropdown", ""),
...
function getCaretPosition(e) {
    ...
    if (document.selection) {
        ...
        var a = e.createTextRange(),
            n = a.duplicate();
        return a.moveToBookmark(t.getBookmark()),
        n.setEndPoint("EndToStart", a),
        n.text.length
    }
    return 0
}
function setCaretPosition(e, t) {
    if (null != e) 
        ...
        e.selectionStart || 0 == e.selectionStart ? (e.focus(), e.setSelectionRange(t, t)) : e.focus()    
}
...
function replaceNewlines_html(e, t) {
    return e.replace(/(\r\n|\n|\r)/g, '<span class="regex_newline">&not;</span>' + (
    t ? "$1" : "")).replace(/\f/g, "↡")
}
...
$("#settings_popup_contents select option").each(function () {
    ...
    e && $(this).prop("selected", "default" === $(this).val())
})}
...
$(".run_tests").attr("original-title", "Run tests"),
$(".test_builder a.button").removeClass("disabled"),
$(".unit_test span.right, .unit_result").removeClass("fail pass").text("n/a"),
cancelUnitTest()}
...
function parseSub(e, t, a, n) {
    ...
    if (/^\\\d+$/.test(e) && getFlavor() !== FLAVOR.JS || /^\$\d+$/.test(e) && getFlavor() !== FLAVOR.PYTHON) {
        ...
        if (a) 
            return n.length > l ? n[l].content : "";
        
        var h = "Replace with content captured in the <i>" + ord(l) + "</i> subpattern.";
        return 0 === l && (h = "Replace with the overall match result."),
        r(e, s("Backreference", h))
    }
    if ("$&" === e) 
        return r(e, s("Backreference", "Replace with the overall match result."));

    if ("$`" === e) 
        return r(e, s("Backreference", "Replace with the portion of the source string that precedes the match."));

    if ("$'" === e) 
        return r(e, s("Backreference", "Replace with the portion of the source string that follows the match."));
...
invalid_browser = !0;flexTypes.forEach(function (e) {
    return detector.style.display = e,
    detector.style.display === e ? void(invalid_browser = !1) : void 0
}),
function () {
    var e,
        t;
    ...
    var e = function () {
        ...
        n = function () {
            ...
            for (var c = ""; ++ s < i;) {
                ...
                if (!isFinite(l) || 0 > l || l > 1114111 || a(l) != l) 
                    throw RangeError("Invalid code point: " + l);
                
                65535 >= l ? o.push(l) : (l -= 65536, e =( l >> 10) + 55296, n = l % 1024 + 56320, o.push(e, n)),
                (s + 1 == i || o.length > r) && (c += t.apply(null, o), o.length = 0)
            }
            return c
        };
