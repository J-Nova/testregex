var Regex101Colorizer = function () {
    ...
    function r(e) {
        if (e.length > 1 && "\\" === e.charAt(0)) {
            var r = e.slice(1);
            if (/^c[\u0000-\u007F]$/.test(r))
                return 64 ^ r.charAt(1).toUpperCase().charCodeAt(0);

            if (/^x(?:[0-9A-Fa-f]{2}|{[0-9A-Fa-f]+})|u[0-9A-Fa-f]{4}$/.test(r))
                return parseInt(r.replace(/[^0-9a-f]/gi, ""), 16);

            if (/^(?:[0-3][0-7]{1,2}|[1-7][0-7]|0)$/.test(r))
                return parseInt(r, 8);

            if (1 === r.length && "cuxDdSsWwhHvVg".indexOf(r) > -1)
                return 0 / 0;
    ...
    function t(t, o, p, i, s) {
        ...
        if (1 === e("posix", N) && /^\[:.*?:\]$/.test(t))
            ...
            for (a =
            {
                ...
                if (c = l[0], "\\" === c.charAt(0))
                    if (/^\\[cuxE]$/.test(c) && 1 === e(c.charAt(1), N))
                        y.push({
                            contents: c,
                            error: !0,
                    ...
                    else if (/^\\[NlLUu]$/.test(c) && -1 === e(c.charAt(1), N))
                        y.push({
                            contents: c,
                            error: !0,
                    ...
                    else if (/^\\[dswb]$/i.test(c))
                        y.push({ contents: c, type: regex_type.META }),
                            h = {
                                rangeable: !1,
                                type: regex_type.META
                            };
                        ...
                        var d;
                        d = 2 === c.length || "{" === c.charAt(2) && "}" !== c.slice(-1) ? error_type.INCOMPLETE_TOKEN : error_type.UNKNOWN_SCRIPT,
                            y.push({
                                contents: c,
                                type: regex_type.META,
                                error: !E.test(c),
                        ...
                        i += "\\Q",
                            "\\E" === f && (x -= 2);
                        for (var A = c.substring(2, x), O = A.split(""), R = 0, b = O.length; b > R; R++)
                            y.push({ contents: O[R], type: regex_type.QUOTE_TEXT, modifiers: a.modifiers });
                        ...
                        y.push({ contents: c, type: regex_type.OCTAL }),
                            h = {
                        ...
                        -1 !== "abfntrehvHV".indexOf(c.charAt(1)) ? (y.push({ contents: c, type: regex_type.META }), h =
                        {
                            rangeable: h.type !== regex_type.RANGE_HYPHEN,
                            charCode: r(c)
                        }) : (y.push({ contents: c, type: regex_type.ESCAPED_TEXT }), h =
                        {
                            rangeable: h.type !== regex_type.RANGE_HYPHEN,
                            charCode: r(c)
                        });
                    ...
                    L.error = !/^\[:\^?(?:alnum|ascii|alpha|word|blank|cntrl|digit|graph|lower|print|punct|space|upper|xdigit):\]$/.test(c),
                        L.error_type = error_type.INVALID_POSIX,
                        y.push(L),
                        h = {
                            type: regex_type.POSIX,
                            rangeable: !1
                ...
                var S = y[y.length - 1];
                S.error && (I = !0),
                    S.modifiers = a.modifiers
            }
        return a.error || (a.group_contents += "]"),
            a
    }
    function n(e, r, t) {
        for (var n, o = r.lastIndex, p = 0; e > p; p++)
            n = r.exec(t);
    ...
    function o(r, n, o) {
            ...
            for (var p = t, i = 0, l = e.length; l > i; i++) {
                ...
                if (!c.error) {
                    if (c.type === regex_type.TEXT) {
                        var _ = c.contents;
                        -1 !== v.modifiers.indexOf("x") && (_ = _.replace(/\s+/g, "")),
                            p += _.length
                    } else if (c.type === regex_type.ESCAPED_TEXT)
                        p += c.contents.length - 1;
                    else if (c.type === regex_type.CHARCLASS)
                        p++;
                    else {
                        if (c.type !== regex_type.META && c.type !== regex_type.HEX && c.type !== regex_type.OCTAL) {
                            if (c.type === regex_type.LOOKAHEAD || c.type === regex_type.LOOKBEHIND)
                                continue;
                        ...
                        if (/^(?:[\^$]|\\[GAZzBbK])$/.test(c.contents))
                            continue;

                        G && "\\C" === c.contents && a(n, 99, -1),
                            p++
        ...
        for (var K = !1; c = y.exec(r);)
            if (_ = c[0], u = _.charAt(0), g = _.charAt(1), K) {
                if ("\n" === _ || "\r" === _)
                    ...
                    K = !1;
                    var $ = _.split("\n");
                    v.children[v.children.length - 1].contents += $[0],
                        y.lastIndex = y.lastIndex - (_.length - $[0].length);
                    continue
                }
                ...
                if ("[" === u) {
                    ...
                    v.children.push(V),
                        v.group_contents += V.group_contents,
                        w += V.group_contents
                } else if ("|" === _) {
                    var Y = {
                        unmatched: !1,
                    ...
                    v.group_contents += _,
                        v.alternations++,
                        N !== FLAVOR.JS && (Y.error = -1 !== v.maxalt && v.alternations > v.maxalt, Y.error_type = error_type.TOO_MANY_ALTERNATIONS),
                        Y.selected = v.selected || isGroupType(v.type) && o == c.index,
                        Y.selected && (v.selected = !0),
                        (void 0 === H && 0 === c.index || void 0 !== H && H.type === regex_type.ALTERNATOR) && (Y.error = !0, Y.error_type = v.level > 0 ? error_type.TRUNCATING_ALTERNATOR_GROUP : error_type.TRUNCATING_ALTERNATOR),
                        v.type === regex_type.SUBPATTERN_GROUP ? (R = v.capturingGroupCount, v.currSubpatterns = []) : v.type === regex_type.CONDITIONAL && (w += "", w += repeat("", v.level * U)),
                        v.children.push(Y)
                } else if ("^" === _ || "$" === _ || "." === _)
                    v.group_contents += _,
                        v.children.push({
                    ...
                    if (x = /^\(\*[\w:]*\)?/.exec(_)) {
                        ...
                        v.lookbehind && (Z.error = !0, Z.error_type = error_type.BACKREF_IN_LOOKBEHIND),
                            v.children.push(Z),
                            M.push(Z)
                    } else if (/^\(\?[imsxXUJ-]*\)$/.test(_))
                        v.group_contents += _,
                            v.modifiers = p(i(v.modifiers, _.substring(2, _.length - 1))),
                            S = -1 !== v.modifiers.indexOf("x"),
                            F = -1 !== v.modifiers.indexOf("U"),
                            D = -1 !== v.modifiers.indexOf("J"),
                            v.children.push({
                                contents: _,
                        ...
                        if (l(v, W), "(" === _)
                            W.type = regex_type.CAPTURING_GROUP,
                                R++,
                                W.ref_num = R,
                                v.type === regex_type.SUBPATTERN_GROUP && (W.subpatterns = v.subpatterns, W.currSubpatterns = v.currSubpatterns, W.currSubpatterns.push(""), W.subpatterns.push(""));
                        else if (_.length > 3 && /^\(\?[imsxXUJ-]*:$/.test(_))
                            W.modifiers = p(i(v.modifiers, _.substring(2, _.length - 1))),
                                S = -1 !== W.modifiers.indexOf("x"),
                                F = -1 !== W.modifiers.indexOf("U"),
                                D = -1 !== W.modifiers.indexOf("J"),
                                W.type = regex_type.GROUP_MODIFIER;
                        else if ("(?|" === _)
                            W.type = regex_type.SUBPATTERN_GROUP,
                                W.capturingGroupCount = R,
                                W.subpatterns = [],
                                W.currSubpatterns = [];
                            ...
                            if ("<" === ee.charAt(0) ? (ee = ee.substring(1), W.type = regex_type.NAMED_P_GROUP) : W.type = regex_type.NAMED_GROUP, isNaN(ee.charAt(0)) || (W.error = !0, W.error_type = error_type.INVALID_GROUP_NAME), void 0 !== v.subpatterns) {
                                ...
                                -1 === O.indexOf(ee) || D ? O.push(ee) : (W.error = !0, W.error_type = error_type.DUPLICATE_SUBPATTERN);
                            R++,
                                W.ref_num = R,
                                W.subpatterns = v.subpatterns,
                                W.currSubpatterns = v.currSubpatterns
                        } else
                            /\(\?<?[=!]$/.test(_) ? ("<" === _.charAt(2) ? (W.type = regex_type.LOOKBEHIND, W.lookbehind = !0, B.push(W)) : (W.type = regex_type.LOOKAHEAD, W.lookbehind = !1), W.quantifiable = !1) : "(?" === _ ? (W.type = N === FLAVOR.JS ? regex_type.GROUP : regex_type.CONDITIONAL, W.maxalt = 1, W.error = !0, W.error_type = error_type.INVALID_GROUP_STRUCTURE) : "(?(DEFINE)" === _ ? (W.maxalt = 0, W.quantifiable = !1, W.type = regex_type.DEFINE) : "(?>" !== _ && "(?:" !== _ && (W.error = !0, W.error_type = error_type.INVALID_GROUP_STRUCTURE);
                        S && v.type !== regex_type.CONDITIONAL && (w += "", w += repeat("", v.level * U)),
                            k.push(w),
                            w = "",
                            v.children.push(W),
                            w += W.contents,
                            S && W.type !== regex_type.CONDITIONAL && (w += "", w += repeat("", W.level * U)),
                            v = W,
                            A.push(v)
                    }
                } else if (")" === u)
                    if (0 === v.level)
                                ...
                                if (d.test(te.group_contents)) {
                                    if (v.condition = te, v.error = !1, te.children.length > 0 && te.children[0].type === regex_type.QUANTIFIER && (te.children[0].type = regex_type.TEXT, te.children[0].error = !1, I = te.children[0].global_error), te.type !== regex_type.LOOKAHEAD && te.type !== regex_type.LOOKBEHIND) {
                                        ...
                                        for (var ne = 0, oe = te.children.length; oe > ne; ne++)
                                            te.contents += te.children[ne].contents;
                        ...
                        S && (w += "", w += repeat("", v.level * U)),
                            w += ")",
                            te.forceSplit || te.group_contents.length > L || te.alternations > 2 && te.group_contents.length > L / 3 || (w = w.replace(/[\x03\x01]/g, "")),
                            w = k.pop() + w,
                            S = -1 !== v.modifiers.indexOf("x"),
                            F = -1 !== v.modifiers.indexOf("U"),
                            S && (w += "", w += repeat("", v.level * U)),
                        ...
                        if (/^\d/.test(g)) {
                            ...
                            if (0 != g && ie > 0)
                                if (10 > ie) {
                                    ...
                                    v.lookbehind && (se.error = !0, se.error_type = error_type.BACKREF_IN_LOOKBEHIND),
                                        v.children.push(se),
                                        M.push(se)
                                }
                                else {
                            ...
                            if (-1 !== "uLlUE".indexOf(g) && -1 === e(g, N))
                                ce.error = !0,
                                    ce.error_type = error_type.UNSUPPORTED_TOKEN;
                            else if (/^\\[cxkguE]$/.test(_))
                                ce.error = !0,
                                    ce.error_type = error_type.INCOMPLETE_TOKEN;
                            else if ("GAZzBbK".indexOf(g) > -1)
                                ce.quantifiable = !1;
                            else if ("x" === g) {
                            ...
                            } else if ("pP".indexOf(g) > -1)
                                ce.error = !E.test(_),
                                    ce.error_type = 2 === _.length || "{" === _.charAt(2) && "}" !== _.slice(-1) ? error_type.INCOMPLETE_TOKEN : error_type.UNKNOWN_SCRIPT;
                            else if ("Q" === g) {
                                ce.type = regex_type.QUOTE,
                                    ce.contents = "\\Q",
                                    w += "\\Q",
                                    v.children.push(ce);
                                var ye,
                        ...
                        if (H)
                            if (H.type !== regex_type.QUANTIFIER && H.type !== regex_type.GROUP_QUANTIFIER || void 0 === H.prev || void 0 === H.prev.quantifier)
                                if (H.error)
                                    Te.error = !0,
                                        Te.error_type = error_type.NOT_QUANTIFIABLE;


                                else if (H.quantifiable) {
                                    var Ne = /^{([0-9]+)(,([0-9]*))?/.exec(_);
                                    if (Ne && (H.from = Ne[1], H.to = Ne[2] ? Ne[3] : Ne[1]), Ne && (+ Ne[1] > 65535 || Ne[3] && + Ne[3] > 65535))
                                        Te.error = !0,
                                            Te.error_type = error_type.TOO_LARGE_QUANTIFIER;
                                    else if (Ne && Ne[3] && + Ne[1] > + Ne[3])
                                        Te.error = !0,
                                            Te.error_type = error_type.BAD_QUANTIFIER_RANGE;
                                    else if (v.lookbehind && !/^{(\d+)(?:,\1)?}$/.test(_))
                                        Te.error = !0,
                                            Te.error_type = error_type.LOOKBEHIND_QUANTIFIER;
                                    else {
                                        ...
                                        H.quantifier = _,
                                            H.quantifier_token = Te,
                                            H !== v && H.group_contents && (H.group_contents += _)
                                    }
                                } else
                                    Te.error = !0,
                                        Te.error_type = error_type.NOT_QUANTIFIABLE;
                            else
                                H.type === regex_type.GROUP_QUANTIFIER && (Te.type = regex_type.GROUP_QUANTIFIER, Te.level = H.level, H.prev.group_contents += _),
                                    Te.error = H.error || !h.test(H.contents + _),
                                    Te.error_type = H.error_type || error_type.NOT_QUANTIFIABLE,
                                    Te.selected = H.selected,
                                    H.prev.quantifier += _,
                                    H.Umode = F;
                        else
                            Te.error = !0,
                                Te.error_type = error_type.NOT_QUANTIFIABLE;
                        v.children.push(Te),
                            H && isGroupType(H.type) && (Te.modifiers = H.modifiers)
                    } else
                    v.group_contents += _,
                        v.children.push({
                            contents: _,
                ...
                var Ie = A.pop();
                Ie.error = !0,
                    Ie.error_type = error_type.UNBALANCED_GROUP,
                    I = !0
            ...
            w = k.pop() + w; m = w; for (var ne = 0, be = B.length; be > ne; ne++)
            s(B[ne].children, -1, 0, B[ne], !0); return C = !0, A
    } function p(e) { return e.split("-")[0] } function i(e, r) {
        function t(e, r) {
            for (var t = r.split(""), n = e; t.length;)
                n = n.split(t.pop()).join("");
            ...
            i = "",
            s = (e || "").split("-"),
            a = (r || "").split("-"); n = s[0], o = a[0], s.length > 1 && (p = s[1]), a.length > 1 && (i = a[1]); var l = t(n, i),
                c = t(p, o); return o += l, i += c, i ? o + "-" + i : o
    } function s() {
        switch (N) {
            case FLAVOR.JS: y = /\[\^?(?:[^\\\]]+|\\[\S\s]?)*]?|\\(?:0(?:[0-3][0-7]{0,2}|[4-7][0-7]?)?|[1-9][0-9]*|x[0-9A-Fa-f]{2}|u[0-9A-Fa-f]{4}|c[A-Za-z]|[\S\s]?)|\((?:\?[:=!]?)?|(?:[?*+]|\{[0-9]+(?:,[0-9]*)?\})\??|[^.?*+^${[()|\\/\r\n]+(?![+*?]|{[0-9]+(?:,[0-9]*)?})|[^.?*+^${[()|\\/\r\n]|[\S\s]/g,
                u = /[^\\-]+|-|\\(?:[0-3][0-7]{0,2}|[4-7][0-7]?|x[0-9A-Fa-f]{2}|u[0-9A-Fa-f]{4}|c[A-Za-z]|[\S\s]?)/g,
                g = /^(\[\^?)((?:[^\\\]]+|\\[\S\s]?)*)(]?)$/,
                h = /^(?:[?*+]|\{[0-9]+(?:,[0-9]*)?\})\??$/,
                x = /^[img]*$/;
                break;
            case FLAVOR.PYTHON: y = /\[\^?]?(?:[^\\\]]+|\\[\S\s]?)*]?|\\(?:(?:0[0-3][0-7]{0,2}|[1-3][0-7][0-7])|[1-9][0-9]*|x[0-9A-Fa-f]{2}|[\S\s]?)|\((?:\?(?:(?:<[!=]|[!:=])!?|P<\w+>|P=\w+\)?|#[^\)]+\)?|[imsux]+\))?)?|(?:[?*+]|\{[0-9]+(?:,[0-9]*)?\})\??|[^#.?*+^${[()|\\/\r\n]+(?![+*?]|{[0-9]+(?:,[0-9]*)?})|[^#.?*+^${[()|\\/\r\n]|[\S\s]/g,
                u = /[^\\-]+|-|\\(?:0[0-3][0-7]{0,2}|[1-3][0-7][0-7]|x[0-9A-Fa-f]{2}|[\S\s]?)/g,
                g = /^(\[\^?)(\]?(?:[^\\\]]+|\\[\S\s]?)*)(]?)$/,
                h = /^(?:[?*+]|\{[0-9]+(?:,[0-9]*)?\})\??$/,
                d = /^\(\w+\)$/,
                x = /^[imgsxu]*$/;
                break;
            default: y = new RegExp("\\[\\^?(?:\\\\Q\\\\E)?]?(?:\\\\Q(?:(?!\\\\E)[\\s\\S])*(?:\\\\E)?|\\[:\\^?[^:]*:\\]|[^\\\\\\]]|\\\\[\\S\\s]?)*]?|\\\\(?:N(?:{\\w*}?)?|k(?:'(?:\\w+'?)?|<(?:\\w+>?)?|{(?:\\w+}?)?)|g(?:[1-9]\\d*|{(?:-?[0-9]+}?|(?:\\w+}?)?)|<(?:[+-]?\\d+>?|(?:\\w+>?)?)|'(?:[+-]?\\d+'?|(?:\\w+'?)?))|Q(?:(?!\\\\E)[\\s\\S])*(?:\\\\E)?|[0-3][0-7]{1,2}|[1-7][0-7]|0|[1-9][0-9]{0,2}|x(?:[0-9A-Fa-f]{2}|{[0-9A-Fa-f]+})?|c[\\u0000-\\u007F]|[pP](?:C[cfnos]?|L[lmotu&]?|M[cen]?|N[dlo]?|P[cdefios]?|S[ckmo]?|Z[lps]?|\\{\\^?(?:\\w+\\}?)?)|[\\S\\s]?)|\\((?:\\?(?:\\(DEFINE\\)|<?[=!]|'(?:\\w+'?)?|P?<(?:\\w+>?)?|(?:[imsxXUJ-]*:|[imsxXUJ-]*\\))|(?:P[=>]\\w*|[0R]|[+-]?[1-9]\\d*)\\)?|\\#[^)]*\\)?|[!|>:]|&\\w*\\)?)?|\\*[:\\w]+\\)?)?|(?:[?*+]|\\{[0-9]+(?:,[0-9]*)?\\})[+?]?|[^#.?*+^{$[()\\\\" + T + "|\\r\\n](?!(?:[?*+]|\\{[0-9]+(?:,[0-9]*)?\\})[+?]?)|[\\S\\s]", "g"),
                u = /\\(?:Q(?:(?!\\E)[\s\S])*(?:\\E)?|[0-3][0-7]{1,2}|[1-7][0-7]|0|x(?:[0-9A-Fa-f]{2}|\{[0-9A-Fa-f]+\})?|c[\x00-\x7F]|[pP](?:(?:C[cfnos]?|L[lmotu&]?|M[cen]?|N[dlo]?|P[cdefios]?|S[ckmo]?|Z[lps]?)|\{\^?(?:\w+\}?)?)|[\S\s]?)|\[:\^?[^:]*:\]|[\S\s]/g,
                g = /^(\[\^?)((?:\\Q\\E)?\]?(?:\\Q(?:(?!\\E)[\s\S])*(?:\\E)?|\[:\^?[^:]*:\]|[^\\\]]+|\\[\S\s]?)*)(]?)$/,
                h = /^(?:[?*+]|\{[0-9]+(?:,[0-9]*)?\})[+?]?$/,
                d = /^\((?:\?<?[=!]|(?:(?:R(?:[1-9]\d*|&\w+)?|[1-9]\d*|[+-]?[1-9]\d*|\w+|'\w+'|<\w+>)\)$))/,
                f = /\\(?:k(?:'\w+'|<\w+>|{\w+})|g(?:[1-9]\d*|{(?:-?[0-9]+|\w+)}|<(?:[+-]?[0-9]+|\w+)>|'(?:[+-]?[0-9]+|\w+)'))/,
                x = /^[xXsiuUmgADJ]*$/,
    ...
    } function a(e, r, t) { e[r] || (e[r] = ""), e[r] = t + e[r] } function l(e, r, t) { e[r] || (e[r] = ""), e[r] += t } function c(e, r, t, n, o, p) {
        function i(e, r, t) {
            ...
            var n = "g" + (
                (r.level - 1) % PAREN_COUNT + 1
            );
            r.selected && (n += " selected_paren");
            var o;
            return o = r.ref_num ? '<b class="' + n + '" ' + t + ' data-groupNum="' + r.ref_num + '">' : '<b class="' + n + '" ' + t + ">",
                i(e, o, "</b>")
        } for (var _ = 0, y = e.length; y > _; _++) {
            var u = e[_],
                g = u.contents;
            if (n || t.injected)
                g = white_space(escapeHtml(g));
            else if (t.currentPos += u.contents.length, t.currentPos > t.pos) {
                var h = g.length - (t.currentPos - t.pos);
                g = white_space(escapeHtml(g.slice(0, h))) + '<div class="caret">&nbsp;</div>' + white_space(escapeHtml(g.slice(h))),
                    t.injected = n = !0
            } else
                g = white_space(escapeHtml(g));
            ...
            if (r && (u.selected = !1), u.error && (g = s(g, d)), isGroupType(u.type)) {
                if (l(p, o.pos, a(g, u, d)), o.pos += u.contents.length, c(u.children, r, t, n, o, p), !u.unmatched) {
                    var f = ")";
                    n || t.injected || t.currentPos != t.pos || (f = '<div class="caret">&nbsp;</div>)', t.injected = !0),
                        u.error ? l(p, o.pos, s(f, d)) : l(p, o.pos, a(f, u, d)),
                        t.currentPos++,
                        o.pos++
                }
                ...
                switch (u.type) {
                    case regex_type.CHARCLASS:
                        if (u.selected ? l(p, o.pos, i(g, '<i class="selected_paren" ' + d + ">", "</i><i>")) : l(p, o.pos, i(g, "<i " + d + ">", "</i><i>")), o.pos += u.contents.length, c(u.children, r, t, n, o, p), colorizeRegex() && (u.selected ? l(p, o.pos, '</i><i class="selected_paren" ' + d + ">") : l(p, o.pos, "</i><i " + d + ">")), !u.error) {
                            var E = "]";
                            n || t.injected || t.currentPos != t.pos || (E = '<div class="caret">&nbsp;</div>]', t.injected = !0),
                                l(p, o.pos, E),
                                t.currentPos++,
                                o.pos++
                        }
                    ...
                    case regex_type.TEXT: l(p, o.pos, g);
                        break;
                    case regex_type.ESCAPED_TEXT: l(p, o.pos, i(g, '<b class="et" ' + d + ">", "</b>"));
                        break;
                    case regex_type.GROUP_COMMENT:
                    case regex_type.COMMENT: l(p, o.pos, i(g, '<b class="comment" ' + d + ">", "</b>"));
                        break;
                    case regex_type.VERB:
                    case regex_type.MODIFIERS:
                    case regex_type.REFERNCE_GROUP: l(p, o.pos, i(g, "<u " + d + ">", "</u>"));
                        break;
                    case regex_type.ALTERNATOR: u.level > 0 ? l(p, o.pos, a(g, u, d, "")) : l(p, o.pos, i(g, "<b " + d + ">", "</b>"));
                        break;
                    case regex_type.GROUP_QUANTIFIER: l(p, o.pos, a(g, u, d, ""));
                        break;
                    default: l(p, o.pos, i(g, "<b " + d + ">", "</b>"))
                }
        ...
        }; !t && highlightInteraction() && $("#richtext_test_colors span").removeClass("selected_paren_test"); var p = {
            pos: 0
        },
            i = {}; c(e, r, o, t, p, i), n && void 0 !== debugHtml && Object.keys(debugHtml).forEach(function (e) {
                a(i, e, debugHtml[e])
            }); var s = ""; if (Object.keys(i).forEach(function (e) {
                s += i[e]
            }), !t) {
            if ("" === s)
